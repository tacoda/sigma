...
- **L3 — Layers as first-class extension.** ✅ Exported the tool seam (`agent.Invoker` / `ToolCall` / `ToolLayer` / `InvokerFunc`); `Config.ToolLayers` compose outermost. `plugin.Host.AddToolLayer(name, layer)` lets plugins contribute layers, threaded through `app.Deps` → main/tui/eval and inherited by sub-agents. First net-new layer plugin: `dryrun` (suppresses mutating tools → plan preview). `sigma layers` shows charter-contributed layers in the spine; a charter A/B (e.g. dryrun on/off) runs on the eval harness. Model/turn spines follow the identical pattern (exported `LLMLayer`/`TurnLayer` + `Config.{LLM,Turn}Layers` + `Host.Add{LLM,Turn}Layer`) when needed. Explicit charter-declared *reordering* is future (layers currently: built-in stack, then contributed layers outermost in registration order).
